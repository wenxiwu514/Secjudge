...
def knn_precision_recall_features(ref_features, eval_features, nhood_sizes=[1000],
...
class FeatureMetric:
    def __init__(self, re_fea, eval_fea, k=5, device='cuda:0'):
        # 将数据转换为 PyTorch 张量并移动到 GPU
        self.re_fea = torch.tensor(re_fea, dtype=torch.float32).to(device)
        self.eval_fea = torch.tensor(eval_fea, dtype=torch.float32).to(device)
        self.k = k
        self.device = device

    def _compute_f_batch(self, phis, Phi):
        distances = torch.cdist(phis, Phi)
        kth_distances = torch.kthvalue(distances, self.k + 1, dim=1).values
        phi_prime_kth_distances = torch.kthvalue(torch.cdist(Phi, Phi), self.k + 1, dim=1).values
        condition = distances <= phi_prime_kth_distances
        return (condition.sum(dim=1) > 0).float()

    def compute_precision(self):
        self.precision = self._compute_f_batch(self.eval_fea, self.re_fea).mean().item()
        return self.precision

    def compute_recall(self):
        self.recall = self._compute_f_batch(self.re_fea, self.eval_fea).mean().item()
        return self.recall
    def compute_f1(self):
...
def compute_fid(synthetic_features, all_private_features, feature_extractor='', folder='', step=0, log_online=False):
    ...
    featureMetric = FeatureMetric(all_private_features, synthetic_features, k=5)
    precision = featureMetric.compute_precision()
    recall = featureMetric.compute_recall()
    f1 = featureMetric.compute_f1()
    print(f'FID: {fid}, Precision: {precision}, Recall: {recall}, F1: {f1}')
    return fid, precision, recall, f1
